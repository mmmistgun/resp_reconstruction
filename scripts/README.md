...
5. `rr_peak_abs_error` 保留为原始尖峰诊断；`band_limited_corr`、`best_lag_corr`、`best_lag_sec` 用于解释低频波形形态和小范围时移，不能单独作为通过标准。
...
`docs/experiments/rawish_state_aligned_l0_l1.md`。2026-06-18 起模型选择以
`rr_peak_band_abs_error` 为主护栏，`rr_peak_abs_error` 只作为原始尖峰和局部毛刺
诊断指标；`band_limited_corr`、`best_lag_corr` 和 `best_lag_sec` 只作为低频形态
和残余时移诊断指标。
...
默认按 `rr_peak_abs_error` 从大到小优先绘制，便于先看原始尖峰和峰谷形态最差的窗口。
