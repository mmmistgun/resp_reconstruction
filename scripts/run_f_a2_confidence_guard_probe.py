...
def _run_root(label: str, branch_mode: str) -> str:
    return f"runs/f_a2_confidence_guard/{_slug(label)}/{branch_mode}"
...
def build_run_specs() -> list[dict]:
    """生成 F-A2 低置信加权 probe：复用旧结果，只训练新 weighting 候选。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="F-A2 confidence-weighted STFT loss guard probe 编排")
