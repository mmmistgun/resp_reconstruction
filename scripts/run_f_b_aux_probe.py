...
FB_AUX_LOSS_OVERRIDES = [
    "loss.fb_stft_win_length=3000",
    "loss.fb_stft_hop_length=500",
    "loss.fb_stft_n_fft=3000",
    "loss.fb_stft_center=false",
    "loss.fb_stft_low_hz=0.033",
    "loss.fb_stft_high_hz=3.0",
    "loss.fb_stft_main_low_hz=0.067",
    "loss.fb_stft_main_high_hz=1.2",
    "loss.fb_stft_diag_weight=0.1",
    "loss.fb_consistency_detach_aux=true",
]
...
ARMS = [
    {
        ...
        "fb_aux_weight": 0.0,
        "fb_consistency_weight": 0.0,
        "fb_consistency_start_epoch": 1,
    ...
    },
    {
        ...
        "fb_consistency_weight": 0.0,
        "fb_consistency_start_epoch": 1,
        "run_root": "runs/f_b_aux/f_b0_aux_enc1/dual",
        "train": True,
    },
    {
...
def build_run_specs() -> list[dict]:
    """生成 F-B0/F-B1 auxiliary probe manifest；F0 anchor 默认复用既有 run。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="F-B auxiliary target-STFT probe 编排")
    ...
    parser.add_argument("--manifest", default="runs/f_b_aux_manifest.csv")
