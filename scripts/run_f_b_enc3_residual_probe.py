...
try:
    ...
    from scripts.run_f_b2_residual_probe import FB_RESIDUAL_MODEL_OVERRIDES
    ...
    from run_f_b2_residual_probe import FB_RESIDUAL_MODEL_OVERRIDES


FB_ENC3_RESIDUAL_MODEL_OVERRIDES = [
    "model.fb_residual_head=enc3_tfgrid_residual",
...
ARMS = [
    ...
    {
        "label": "F-B3_enc3_tfgrid_residual",
        "branch_mode": "dual",
        "stft_inject_position": "pre_mixer",
        "paired_f0_label": "F0_native_stft_pre_mixer",
        "paired_time_only_label": "F0_native_time_only",
        ...
        "fb_residual_scale": 0.03,
        "fb_residual_energy_cap": 0.0,
        "run_root": "runs/f_b_enc3_residual/f_b3_enc3_tfgrid_residual/dual",
        "train": True,
        **F_B1_LOSS,
    },
    {
        "label": "F-B3b_enc3_tfgrid_residual_cap",
        "branch_mode": "dual",
        "stft_inject_position": "pre_mixer",
        "paired_f0_label": "F0_native_stft_pre_mixer",
        "paired_time_only_label": "F0_native_time_only",
        ...
        "run_root": "runs/f_b_enc3_residual/f_b3b_enc3_tfgrid_residual_cap/dual",
        "train": True,
        **F_B1_LOSS,
    },
]
...
def _residual_overrides(spec: dict) -> list[str]:
    if spec["fb_residual_head"] == "low_complex_residual":
        return [*FB_RESIDUAL_MODEL_OVERRIDES]
    if spec["fb_residual_head"] == "enc3_tfgrid_residual":
        return [
            *FB_ENC3_RESIDUAL_MODEL_OVERRIDES,
            f"model.fb_residual_energy_cap={spec['fb_residual_energy_cap']}",
        ]
    return []
...
def _base_spec(arm: dict, seed: int) -> dict:
    ...
    if residual_enabled:
        ...
        overrides.extend(_residual_overrides(arm))
    return {**arm, "seed": seed, "overrides": overrides}


def build_run_specs() -> list[dict]:
    """生成 F-B Enc3/residual cap probe manifest；F0 与 F-B2 默认复用。"""
...
def manifest_row(spec: dict) -> dict:
    return {
        ...
        "fb_residual_energy_cap": spec["fb_residual_energy_cap"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="F-B Enc3 TF-Grid-lite residual / energy cap probe 编排")
    ...
    parser.add_argument("--manifest", default="runs/f_b_enc3_residual_manifest.csv")
