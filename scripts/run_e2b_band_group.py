...
# stft_encoder_type 是唯一变量：dual 臂用 bandgroup（分频带子图），sanity 用 conv2d（全频带图）。
STFT_BASE = [
...
# E1-D 前 4 seed，保证与 runs/e1_d_warmup_time_only 同 seed 可配对（简单探针力度）。
SEEDS = [20260700, 20260710, 20260837, 20260901]

# sanity 用其中一个 seed 复现 B2-0 dual 全频带数，守住「复用旧 time_only 桩」前提。
SANITY_SEED = 20260700

DUAL_RUN_ROOT = "runs/e2b_band_group_dual"
SANITY_RUN_ROOT = "runs/e2b_fullband_sanity"


def build_run_specs() -> list[dict]:
    """4 个 bandgroup dual + 1 个全频带 conv2d sanity。"""
    specs: list[dict] = []
    for seed in SEEDS:
        specs.append(
            {
                ...
                "overrides": [
                    *STFT_BASE,
                    "model.stft_encoder_type=bandgroup",
...
def _tag(spec: dict) -> str:
    return f"e2b_{spec['kind']}_{spec['seed']}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E2b 分频带子图探针编排（4 bandgroup dual + 1 全频带 sanity）")
    ...
    parser.add_argument("--manifest", default="runs/e2b_band_group_manifest.csv")
