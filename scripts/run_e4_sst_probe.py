...
# E1-D 实验口径：仅覆盖与默认 yaml 不同的项；极性 warmup 已固化进默认 loss 段。
COMMON_OVERRIDES = [
...
# native_inject 固定口径：patch + 8Hz + N0（与 B2-0 对齐，全频带 sanity 可复现 B2-0 dual）。
# 唯一变量：dual 臂用 sst_cached（离线 SST 缓存表征），sanity 用 conv2d（全频带 STFT 图）。
STFT_BASE = [
...
# E1-D 前 4 seed，保证与 runs/e1_d_warmup_time_only 同 seed 可配对。
SEEDS = [20260700, 20260710, 20260837, 20260901]
SANITY_SEED = 20260700

# 默认 SST 缓存路径（预计算脚本产物），可用 --sst-cache 覆盖。
DEFAULT_SST_CACHE = "runs/sst_cache/sst_8hz_37f.npz"
SST_IN_FREQ = 159

DUAL_RUN_ROOT = "runs/e4_sst_dual"
SANITY_RUN_ROOT = "runs/e4_fullband_sanity"


def build_run_specs(sst_cache: str = DEFAULT_SST_CACHE) -> list[dict]:
    """4 个 sst_cached dual + 1 个全频带 conv2d sanity。"""
    specs: list[dict] = []
    for seed in SEEDS:
        specs.append(
            {
                ...
                "overrides": [
                    *STFT_BASE,
                    "model.stft_encoder_type=sst_cached",
                    f"model.stft_sst_in_freq={SST_IN_FREQ}",
                    f"data.sst_cache_path={sst_cache}",
                    f"outputs.run_root={DUAL_RUN_ROOT}",
                    f"training.seed={seed}",
                ],
            }
        )
    # 全频带 sanity：conv2d、不用 sst 缓存，应复现 B2-0 dual 同 seed（runs/b2_native_dual_8hz）。
    specs.append(
        {
            "kind": "sanity",
            "seed": SANITY_SEED,
...
def _tag(spec: dict) -> str:
    return f"e4_{spec['kind']}_{spec['seed']}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E4-SST 探针编排（4 sst_cached dual + 1 全频带 sanity）")
    parser.add_argument("--sst-cache", default=DEFAULT_SST_CACHE, help="预计算 SST 缓存 npz 路径")
    ...
    parser.add_argument("--manifest", default="runs/e4_sst_manifest.csv")
    args = parser.parse_args()
    skipped = set(args.skip)

    specs = build_run_specs(args.sst_cache)
