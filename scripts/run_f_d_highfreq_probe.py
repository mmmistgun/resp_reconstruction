...
DEFAULT_CWT_CACHE = "runs/f_d_highfreq_cache/high_cwt_1_8hz_36x180.npz"
DEFAULT_MODULATION_CACHE = "runs/f_d_highfreq_cache/high_cwt_modulation_1_8hz_8x180.npz"
...
def _arm_with_cache_overrides(arm: dict, *, cwt_cache: str, modulation_cache: str) -> dict:
    arm = dict(arm)
    if arm["label"] == "F-D1_high_cwt":
        arm["cache_path"] = cwt_cache
    elif arm["label"] == "F-D2_high_cwt_modulation":
...
def build_run_specs(
    *,
    ...
    """生成 F-D high-frequency representation probe manifest；F0 anchor 默认复用。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="F-D high-frequency representation probe 编排")
    parser.add_argument("--cwt-cache", default=DEFAULT_CWT_CACHE, help="F-D1 high-CWT 缓存 npz 路径")
    parser.add_argument("--modulation-cache", default=DEFAULT_MODULATION_CACHE, help="F-D2 modulation 缓存 npz 路径")
    ...
    parser.add_argument("--manifest", default="runs/f_d_highfreq_manifest.csv")
