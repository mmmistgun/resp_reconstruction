...
目的：在 B2-0 的 `native_inject` substrate 上，验证“不压缩频率细节、只显式分频带”的先验是否比
完整 2D fullband 图更好。E2b 与 E2c 的关键差异是：E2c 把每个频带压成均值能量序列；
E2b 保留每个频带内的频点，插值到公共 `F=32` 后堆成 `(B,5,F,T)`，再用共享 `Conv2d`
编码，最后沿 `F` 求均得到 `(B,16,T)`。因此 E2b 若持平，含义是“分组先验不必要”，不是
“压缩有害”。

固定配置：

- 模型：`patch_mixer1d + time_stft_dual1d + fusion_mode=native_inject + N0 8Hz`。
- STFT 表征：`encoder_type=bandgroup`，默认 5 个重叠频带，`band_group_f=32`。
- warmup：沿用 E1-C1 默认轻 warmup。
- seeds：`20260700`、`20260710`、`20260837`、`20260901`、`20260911`、`20260920`。
- 输出：
  - 明细：`runs/e2b_band_group_detail.csv`
  - 聚合：`runs/e2b_band_group_grouped.csv`
  - 对 B2-0 配对 delta：`runs/e2b_band_group_vs_b2_paired_delta.csv`
  - 全频带 sanity：`runs/e2b_fullband_sanity/20260623_130812_776937`

sanity：E2b 脚本里的 `conv2d fullband dual` 复现 B2-0 同 seed `20260700`，`frac_gt_1`
为 `0.127103`，B2-0 为 `0.124860`，差值 `+0.22pp`；`frac_gt_2` 为 `0.047103`，
B2-0 为 `0.048224`，差值 `-0.11pp`。差异很小，说明复用 B2-0 fullband 桩的口径成立。

E2b `bandgroup dual` 聚合结果：

| n | mean | median | p95 | `frac_gt_1` | `frac_gt_2` |
|---:|---:|---:|---:|---:|---:|
| 6 | 0.527876 | 0.188396 | 2.107074 | 0.127975 | 0.053520 |

对 B2-0 `fullband dual` 的同 seed 配对 delta（E2b - B2-0）：

| seed | E2b `frac_gt_1` | B2-0 `frac_gt_1` | delta | E2b median | B2-0 median | delta |
|---:|---:|---:|---:|---:|---:|---:|
| 20260700 | 0.134206 | 0.124860 | +0.009346 | 0.193722 | 0.191969 | +0.001753 |
| 20260710 | 0.110280 | 0.121121 | -0.010841 | 0.175027 | 0.181163 | -0.006136 |
| 20260837 | 0.126729 | 0.142430 | -0.015701 | 0.181558 | 0.185510 | -0.003951 |
| 20260901 | 0.120374 | 0.129720 | -0.009346 | 0.195419 | 0.200673 | -0.005255 |
| 20260911 | 0.152523 | 0.149907 | +0.002617 | 0.193722 | 0.190270 | +0.003453 |
| 20260920 | 0.123738 | 0.135701 | -0.011963 | 0.190927 | 0.194137 | -0.003210 |

配对汇总：

| 指标 | 平均 delta | 方向 |
|---|---:|---|
| mean | -0.026535 | 改善（4/6 seed 为负） |
| median | -0.002224 | 基本持平略好（4/6 seed 为负） |
| p95 | -0.113877 | 改善（4/6 seed 为负） |
| `frac_gt_1` | -0.005981 | 改善（4/6 seed 为负，-0.60pp） |
| `frac_gt_2` | -0.004174 | 改善（4/6 seed 为负，-0.42pp） |
...
### E4 前置：STFT / CWT / SST 幅度谱谐波分离对比（不训练，只读数据）

目的：E4 候选频域表征里，先用「纸面/可视化」判断 SST（同步压缩）/CWT 幅度谱相对当前训练
用 STFT 幅度图，在**高误拣窗口**上是否真把「呼吸基频 vs 2× 谐波」分得更开——分离度明显更好
才值得进训练探针，否则直接收，省一整轮。

前提澄清（决定 E4 走向）：dual 的时序分支吃原始 BCG 波形，**相位信息时序分支天然完整保留**。
故「真相位表征」（复数 STFT / IF / 相位差分）在 dual 里大概率**冗余**，已搁置（见“当前决策”）。
真正稀缺的是「更好的频率结构分离」，CWT/SST 幅度谱正是冲此而来——它们不含可直接利用的相位
（SST 的相位只在锐化那步用过、不在幅度输出里），故只解「谐波分离」、不解极性盲。

方法：脚本 `scripts/e4_timefreq_compare.py`，从 `runs/b2_native_dual_8hz` 某 run 的 metrics 挑
6 个最高误拣 + 3 个干净窗口，对 BCG 输入分别算 STFT（沿用训练口径 win=3000/hop=500/hann）、
CWT（复 Morlet `cmor1.5-1.0`，尺度覆盖 0.04-1.0Hz）、SST（ssqueezepy `ssq_cwt`）幅度谱。
判据 = **基频带能量 / 2× 谐波带能量**（基频取 tho 真值 RR±15%、谐波取 2×RR±15%，时间平均），
越大 = 基频越突出 = 越不易误拣。产物（本地，runs/ 不入库）：`runs/e4_timefreq_compare/*.png`
（9 窗三联时频图）+ `separation_summary.csv`。

结果（谐波分离度，越大越好）：

| 窗口 | STFT | CWT | SST |
|---|---:|---:|---:|
| 高误拣均值（6 窗） | 0.650 | 0.589 | **0.911** |
| 干净窗 10735 | 1.683 | 1.399 | 2.401 |
| 干净窗 12127 | 2.371 | 2.169 | 3.128 |
| 干净窗 10698 | 1.154 | 1.219 | 2.414 |

三个硬事实：

1. **误拣根因被实锤**：高误拣窗 STFT 分离度 ≈ `0.65`（基频能量竟**小于** 2× 谐波能量），干净窗
   `1.2~2.4`（基频明显占优）。误拣 = 谐波分离不足，从假设变成实测——这是本次最有价值的产出。
2. **SST 在难窗显著拉开分离度**：`0.650 → 0.911`（+40%），且 6 个难窗**个个都涨**，无一例外。
3. **CWT 反而更差**（`0.589 < 0.650`）：低频带固定窗 STFT 优于小波，**CWT 砍掉，不进 E4**。

保留意见（防止重蹈 E2b/E2c）：

- SST 难窗分离度虽 +40%，但 `0.911 仍 < 1.0`——基频还是没超过谐波，是「改善」非「解决」，
  模型在此分离度下仍可能误拣。
- 分离度↑ ≠ 误拣率↓：中间隔着模型 + dual 的强时序基线；B2-0/E2c/E2b 一路都栽在「频域看着更好、
  喂进 dual 被吃掉打平」。SST 仍是「dual 里换一种 magnitude 表征」，与 E2b/E2c 同质。

判定：**SST 进 E4 探针（有保留的 GO），CWT 砍掉，真相位搁置。** 与 E2b/E2c 立项的关键区别是
SST 有前置实证支撑（难窗 +40%、6/6 一致、直指根因），但仍按探针纪律（native_inject、4 seed、
对 B2-0 配对）跑。**若 SST 探针也打平，则可判定「频域侧在 dual 里到顶，瓶颈在融合/时序分支已够强
而非表征」，作为 E4 终点结论。**

补充观察一（难窗含低频漂移，SST 未必能救）：8Hz 全频带视图下，难窗（如 row 10467）的 `0-50s`
有一块 `<0.2Hz` 的强能量斑——是体动/基线漂移，与呼吸基频混叠。SST 锐化的是「谐波分离」，
对「宽带低频漂移淹没呼吸」这类污染无能为力。故 SST 探针即便有改善，预期也只救回「纯谐波误拣」
那部分窗口，救不回「漂移污染」窗口——预期从「可能明显赢」下调到「可能小赢一部分难窗」。

补充观察二（频带能量分布实测，确认当前设定无问题）：复查「3-8Hz 是否过强、压制了 0.05-0.7Hz
重要信息」。实测难窗各频带 STFT 能量（row 10467）：

| 频带 | 原始幅度均值 | log1p（N0 实际喂模型口径） |
|---|---:|---:|
| 呼吸 0.05-0.7Hz | 138.7 | **4.56（全频带最高）** |
| 2× 谐波 0.7-1.4Hz | 94.6 | 4.36 |
| 心冲击 1.4-3Hz | 34.1 | 3.25 |
| 5-7Hz（次级峰） | 70.8 | 3.82 |

结论：原始幅度上 5-7Hz 确有次级能量峰（心冲击高次谐波/传感器特性），但**在 N0（log1p）口径下
呼吸带 4.56 仍是最强**——log1p 天然压扁高频、不让低频吃亏，「高频压制低频」在喂模型口径下不成立。
两个相关想法的裁决：(1)「砍到 0.05-3Hz」= 重做 E1d 频带扫描，B1 已定 8Hz 是甜点（vs 3Hz/12Hz），
不为 E4 单独砍；(2)「低频强化归一化」= 已实现为 **N1**（per-freq-bin IQR 尺度），E1 已验：N1 3Hz
相对 N0 8Hz 仅微弱改善且引入训练不稳（seed 20260901 gate failure），故主线选 N0。因此
**E4-SST 第一刀沿用 N0 全频带 8Hz、对齐 B2-0 口径不变**；N1-SST 若需要，作为第二旋钮后置（单变量）。

### E4-SST：离线 SST 幅度谱表征探针（打平，E4 终点）

目的：把 SST 幅度谱作为 dual 的 STFT 表征替代，验证「前置实测的 +40% 谐波分离」能否在训练里
兑现成误拣率下降。SST 单窗 ~357ms（STFT 的 ~300 倍）且是固定无参变换，故**离线预计算缓存**
（带限 0.05-8Hz=159 频点、降采样 37 帧均值池化、log1p N0），训练时按 `dataset_row_id` 读 ~23KB/窗。
这是 E 系列首次破「dataset/engine 零改动」——engine 用「batch 含 sst 才传」严格向后兼容。

固定配置（除表征外逐字段等于 B2-0）：`patch + base16 + native_inject + N0 8Hz + 默认 warmup`，
唯一变量 `stft_encoder_type=sst_cached`（缓存 `runs/sst_cache/sst_8hz_37f.npz`，12816 窗全覆盖）。
seeds：`20260700/20260710/20260837/20260901`（4 seed 探针）。

sanity：全频带 conv2d dual seed 20260700 `frac_gt_1=0.1279`，B2-0 同 seed `0.1249`，差 `+0.30pp`
（median/mean 几乎一致）→ 复现成立，**证明 engine 改动未污染非 sst 路径**（sanity 走 `sst=None`
旧路径），dual 配对可信。

对 B2-0 全频带 dual 同 seed 配对 delta（E4-SST − B2-0）：

| seed | E4-SST `frac_gt_1` | B2-0 `frac_gt_1` | delta |
|---:|---:|---:|---:|
| 20260700 | 0.126355 | 0.124860 | +0.0015 |
| 20260710 | 0.118505 | 0.121121 | -0.0026 |
| 20260837 | 0.146542 | 0.142430 | +0.0041 |
| 20260901 | 0.126729 | 0.129720 | -0.0030 |

汇总：**2 负 2 正，均值 `Δfrac_gt_1 ≈ +0.00pp`** —— 彻底打平，比 E2c(+0.50)、E2b(-0.60) 都更贴零，
四 seed 在零附近抖动、无任何方向。

判定：**E4-SST 打平 → E4 终点结论「频域侧在 dual 里到顶，瓶颈在融合/时序分支已够强而非表征」。**
这次打平比 E2b/E2c 更可信、更有信息量：SST 是唯一有前置实证（难窗谐波分离 +40%、6/6 一致、直指
根因）的频域表征，连它都打平，精确验证了保留意见——**分离度↑ ≠ 误拣率↓**：SST 在频谱图上把基频/
谐波分得更开，但时序分支已隐式解决了这部分，显式化是冗余；且 SST 能锐化的谐波分离正是模型已会的，
救不了的低频漂移污染才是剩余误拣真凶。

### E1→E4 频域输入探索整体收口

五个频域实验连成完整闭环，结论一致且互相印证：

| 实验 | 改什么 | Δf1 vs 全频带 | 结论 |
|---|---|---:|---|
| B2-0 | 窄通道注入（换融合解码） | -1.40pp（弱于 concat-deep） | 否决 |
| E2c | 5 带能量（压缩频域） | +0.50pp | 否决 |
| E2b | 5 带子图（分组先验） | -0.60pp | 持平收尾 |
| **E4-SST** | **SST 锐化表征** | **+0.00pp** | **打平，终点** |

**硬结论：`conv2d 全频带 STFT 幅度图 + dual` 是意外地强、且已到顶的配置。** 五次尝试（换融合解码、
压缩频域、分组先验、换更锐时频表征）无一在干净 substrate 上明显超过它。瓶颈实锤定位：**不在频域
表征**（SST 都打平），而在「dual 里时序分支已够强 + 难窗低频漂移污染 SST/STFT 都救不了」。
继续投频域输入/表征（E3 可学习前端、N1-SST、心冲击高分辨率网格）大概率都是此打平结果的变体。
**剩余误拣的真正未碰方向是「难窗低频漂移污染」（去漂移/质量门控），或跳出频域回到时序/损失/下游。**
...
1. 五个实验（B2-0 换融合、E2c 压缩、E2b 分组先验、E4-SST 锐化表征）一致表明：`conv2d 全频带
   STFT 图 + dual` 已到顶，无一明显超过它。详见「E1→E4 频域输入探索整体收口」段。
2. **不再投频域输入/表征**：E3 可学习前端、N1-SST、心冲击高分辨率网格大概率都是此打平结果的变体——
   瓶颈被实锤在「时序分支已够强 + 难窗低频漂移污染」，不在表征。重启需先提出针对「漂移污染」的
   新机制假设，否则视为已闭环。
3. 剩余误拣的真正未碰方向：(a)「难窗低频漂移污染」去除（去漂移前端/质量门控），(b) 跳出频域回到
   时序主干/损失/下游任务。两者均属新阶段，需单独立项。
