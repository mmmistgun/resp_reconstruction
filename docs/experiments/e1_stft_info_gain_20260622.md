...
当前主候选更新为 `patch_mixer1d + conv2d + N0 8Hz + deep + fuse_len=600`。两个混淆因子
核查后：门控已澄清（无误杀，正信号可信）；multiscale 崩坏不是 STFT 导致。**逐窗分布核查
...
- **对 E1 结论是利好**：gate 干净隔开两 basin、fail 是真反相、配对统计只建立在真正同极性的
  run 上，patch 的 4/4 正结论不受影响（前节“疑点 1 续三 / 点 4”的“多稳态非 STFT 独有”由此坐实）。
...
- 降低翻转率属 loss/训练侧干预（极性锚定 / signed 项课程），与 E1 收尾是两条线，留待 B1 之后。
...
- 主候选更新为：`patch_mixer1d + conv2d + N0 8Hz + deep + fuse_len=600`。这是当前 E1
  最干净的正结论配置：9 个成功配对里 `frac_gt_1` 9/9 下降，且 corr 9/9 提升。
...
- `patch_mixer1d + STFT`：**给出当前 E1 的正向结论**。deep+`fuse_len=600`、8Hz N0 的
  ...
  同向改善，且 `band_limited_corr` / `best_lag_corr` 也 9/9 提升。限制从“样本量不足”
  转为“仍有 gate fail/极性多稳态，需要后续训练稳定性修复”。
...
3. 限制：time_only `20260837/20260901` gate fail，dual `20260901` gate fail。B1 已不缺配对数，
   但 wrapper 极性多稳态仍会造成约 1/3 run 无结果，后续要从 loss/训练稳定性侧处理。
4. 下一步方法开发：进入 patch 路线的融合方法优化（门控/注意力/FiLM）时，必须带
   time_only 对照桩，并沿用 `deep+fuse_len=600`，证明涨幅来自“更好地用了 STFT”，而不是
   新结构本身增强了裸 time backbone。
5. trade-off 已复核：8Hz 有效配对里 `band_limited_corr` / `best_lag_corr` 都是 9/9 提升，
   3Hz/12Hz 也都是 4/4 提升。下一轮可把这两列作为固定 sanity，一起进入融合方法优化。
...
3. **E1 已可收尾后再动**：改 loss/init = 改优化景观，会让已完成的 E1a'/E1b 配对不可比。
   因此本轮 E1/B1 不再回改训练配方；下一阶段若处理极性多稳态，应作为单独实验线重启对照。
4. 该改动影响所有实验的训练行为，合入时须单独验证不扭曲 STFT 增益判定。
