...
3. 限制：time_only `20260837/20260901` gate fail，dual `20260901` gate fail。B1 已不缺配对数，
   但 wrapper 极性多稳态仍会造成约 1/3 run 无结果，后续要从 loss/训练稳定性侧处理（见优先级 3）。
...
3. 这是改优化景观的改动，会让已完成的 E1a'/E1b 配对不可比，故必须作为**单独实验线**重启，
   保留现有冻结配方的 E1 结论作为对照基线；不在 E1 记录内回改。
4. 该改动影响所有实验的训练行为，合入时须单独验证**不扭曲 STFT 增益判定**（用同一套
   E1a' vs E1b 误拣率配对口径，确认修复后 `Δf1` 仍稳定为负、且 gate fail 率下降）。
5. 验收目标：在不牺牲 patch `Δf1≈-2.34pp` 正增益的前提下，把约 1/3 的 gate fail 率显著降低
   （理想到接近 0），使后续实验不再需要 ~2× seed 冗余。
