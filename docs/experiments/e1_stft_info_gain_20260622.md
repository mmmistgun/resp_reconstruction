...
当前可保留的候选方向是 `patch_mixer1d + conv2d + N1 3Hz`，它在主指标上略优于 N0 3Hz，
且 seed 间方差较小。但这个收益仍需要更多 seed 或独立 split 复核，不能直接升级为稳定结论。
...
但该收益不全面：E1b/E1n1 的 `band_limited_corr` 和 `best_lag_corr` 明显低于 E1a，
`relative_envelope_mae` 也没有同步改善。因此当前只能记录为“主指标局部增益”，
不能判定为“波形重建质量整体改善”。
...
- 保留候选：`patch_mixer1d + conv2d + N1 3Hz`。
- 保留对照：`patch_mixer1d + conv2d + N0 8Hz`。
...
1. 对 `patch_mixer1d + N1 3Hz` 和 `patch_mixer1d + N0 8Hz` 补少量 seed 或独立 split 复核。
2. 抽样绘制代表窗口，检查 peak-band MAE 改善是否以波形相关性下降为代价。
