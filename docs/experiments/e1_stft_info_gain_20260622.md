...
5. 该改动影响所有实验的训练行为，合入推荐配方前须单独验证**不扭曲 STFT 增益判定**（用同一套
   E1a' vs E1b 误拣率配对口径，确认修复后 `Δf1` 仍稳定为负、且 gate fail 率下降）。
6. 剩余验收目标：在不牺牲 patch `Δf1≈-2.34pp` 正增益的前提下，把约 1/3 的 gate fail 率显著降低
   （E1-C 已显示 dual 分支可接近 0），使后续实验不再需要 ~2× seed 冗余。
