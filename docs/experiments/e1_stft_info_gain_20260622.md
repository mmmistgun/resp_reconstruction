...
|---:|---:|---:|---:|---:|---:|
| 4 | 0.497148 | 0.186870 | 1.893283 | 0.120561 | 0.046542 |
...
| 指标 | 平均 delta | 方向 |
|---|---:|---|
| mean | -0.040340 | 改善（4/4 seed 为负） |
| median | -0.002959 | 基本持平（2/4 seed 为负） |
| `frac_gt_1` | -0.008972 | 改善（3/4 seed 为负，-0.90pp） |
| `frac_gt_2` | -0.007009 | 改善（4/4 seed 为负，-0.70pp） |

判定：E2c 不是 2:2 骑墙，不触发补到 6 seed 的应急条件；它在 B2-0 同口径下给出
`frac_gt_1` 3/4 seed 改善、`frac_gt_2` 4/4 seed 改善的正向探针信号。这个结果**不能推翻**
E1-D 作为当前 patch 主线定稿配方，因为 E2c 尚未跑 `time_only` 对照，也没有证明优于
E1-D concat 强基线；但它说明 B2-0 的负结果不应简单归因于 `native_inject` substrate 全无价值，
更具体的问题可能是 fullband 2D STFT 表征过宽/过噪。后续若继续 E2 系列，应优先围绕
“STFT 表征压缩/分带”推进，而不是直接回到 gating/FiLM。
...
优先级 4（patch 融合方法优化，**B2-0 后暂停 gating，E2c 后保留表征探针入口**）：
...
3. 因此 **不继续 B2-1 gating**，除非先提出新的、能解释为何 gating 会弥补 B2-0 增益变弱的机制假设。
   当前默认回到 E1-D 定稿配方推进后续研究，而不是继续扩融合结构。E2c 后唯一保留的入口是
   STFT 表征压缩/分带探针，因为它在同一 B2-0 substrate 上相对 fullband dual 给出正向小样本信号。
...
5. **patch gating/FiLM 扩结构线现整体封存**（不是只停 B2-1）：B2-0 已证明「换更原生解码」方向走不通，
   ...
   **STFT 增益依赖一个跨 token 的宽混合空间**（deep 头在 `fuse_len=600` 上恰好提供了它，
   ...
   patch 主线一律以 E1-D 定稿配方为准。E2c 属于表征侧探针，不等价于重启 gating/FiLM 扩结构线。
