...
- 先跑 split 独立性审计确认当前数据 samp_id 隔离（数据制作阶段已分离，这里固化为
  可复核前置），并在 20260620 soft-z 口径补跑 `downsampled_ssm1d`，把三主线 + 三反例
  凑齐同口径 baseline。
- 然后做 E1：包含 `3/8/12Hz` STFT 频带上限消融，并至少覆盖一个 patch/mixer 类和一个
...
前端（MoE、LEAF、frequency-domain mixer）和 attention 融合才有清晰比较基线。
