...
- `TimeStftDual1D.forward(x) -> Tensor`：训练主路径固定窗口 `(B,1,duration_samples)` → `(B,1,duration_samples)`；单元测试可用同长度小窗口，但 registry 构造的输出长度以 `cfg.window.duration_samples` 为准。
- cfg 字段：`model.name=time_stft_dual1d`、`model.branch_mode∈{time_only,stft_only,dual}`、`model.time_backbone`（主干名）、`model.stft_win`、`model.stft_hop`、`model.stft_low_hz`、`model.stft_high_hz`、`model.stft_out_channels`、`model.stft_encoder_type∈{conv1d,conv2d}`、`model.stft_norm∈{n0,n1}`、`model.fuse_len`、`model.stft_band_scale_path`（N1 用，可空）。
- registry 派生字段：`time_feat_channels`、`out_length`、`stft_kwargs` 不作为直接配置字段暴露；其中 `patch_mixer1d` 的 `time_feat_channels=base_channels`，`multiscale_decomp_mixer1d` 的 `time_feat_channels=base_channels * len(downsample_factors)`。
...
在 `_REGISTRY` 字典中追加一个工厂条目（放在 `downsampled_ssm1d` 之后、闭括号之前）。该工厂解析双分支配置，并按 `time_backbone` 推断时序主干特征通道数：
...
```python
    "time_stft_dual1d": lambda cfg: TimeStftDual1D(
        ...
        time_feat_channels=_time_backbone_feat_channels(cfg),
...
def _time_backbone_feat_channels(cfg: Any) -> int:
    """返回 time_backbone(return_features=True) 的通道数，保证 FusionHead 输入通道匹配。"""
    name = str(cfg.model.get("time_backbone", "patch_mixer1d"))
    base_channels = int(cfg.model.base_channels)
    if name == "patch_mixer1d":
...
def test_n1_phase_builds_three_representative_specs():
    ...
    assert len(specs) == 3
    assert all(s["label"] == "E1n1" for s in specs)
    assert all("model.stft_norm=n1" in s["overrides"] for s in specs)
    assert all("model.stft_band_scale_path=runs/stft_band_scale/band_scale_3hz.npy" in s["overrides"] for s in specs)
...
def test_manifest_rows_preserve_run_factors():
    ...
    assert {"tag", "label", "time_backbone", "high_hz", "encoder_type", "seed"} <= set(row)
```
...
def _plain_backbone_overrides(backbone: str) -> list[str]:
    ...
    if backbone == "patch_mixer1d":
        base += ["model.patch_len=256", "model.patch_stride=128", "model.overlap_window=hann", "model.output_smoothing_kernel=1"]
    return base
...
def build_n1_specs(encoder: str = "conv1d", band_scale_path: str = "runs/stft_band_scale/band_scale_3hz.npy") -> list[dict]:
    """N1 对照：代表档 E1b/patch_mixer1d/high_hz=3 上加挂 3 seed。"""
    specs: list[dict] = []
    for seed in SEEDS:
        specs.append({
            "label": "E1n1", "time_backbone": "patch_mixer1d", "high_hz": 3.0,
            "encoder_type": encoder, "seed": seed,
            "overrides": [*STFT_BASE, f"model.stft_encoder_type={encoder}",
                          "model.time_backbone=patch_mixer1d", "model.branch_mode=dual",
                          "model.stft_high_hz=3.0", "model.stft_norm=n1",
                          f"model.stft_band_scale_path={band_scale_path}", f"training.seed={seed}"],
        })
    return specs
...
def manifest_row(spec: dict) -> dict:
    """生成可落盘的 run 因子行；汇总时用 tag/因子回连时间戳 run 目录。"""
    return {
        "tag": _tag(spec),
        "label": spec["label"],
        "time_backbone": spec["time_backbone"],
        "high_hz": "" if spec["high_hz"] is None else spec["high_hz"],
...
def main() -> None:
    ...
    parser.add_argument("--phase", choices=["zero", "main", "n1"], default="main",
                        help="zero=零号消融选编码器；main=主线 48 run；n1=代表档 N1 对照 3 run")
    parser.add_argument("--encoder", choices=["conv1d", "conv2d"], default="conv1d",
                        help="主线 STFT 编码器（零号消融胜者）")
    parser.add_argument("--band-scale-path", default="runs/stft_band_scale/band_scale_3hz.npy",
    ...
    parser.add_argument("--max-parallel", type=int, default=1,
                        help="并发训练进程数。默认 1；同一 GPU 多实验前先确认显存余量")
    parser.add_argument("--manifest", default="runs/tho_research_v2_20260620_e1_stft_info_gain_manifest.csv")
    args = parser.parse_args()
    skipped = set(args.skip)

    if args.phase == "zero":
        specs = build_zero_ablation_specs()
    elif args.phase == "n1":
        specs = build_n1_specs(args.encoder, args.band_scale_path)
    else:
        specs = build_run_specs(args.encoder)

    manifest_path = Path(args.manifest)
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    with manifest_path.open("w", newline="", encoding="utf-8") as fp:
        writer = csv.DictWriter(fp, fieldnames=["tag", "label", "time_backbone", "high_hz", "encoder_type", "seed", "overrides"])
        writer.writeheader()
...
python scripts/run_e1_stft_info_gain.py --phase n1 --dry-run | wc -l     # 期望 3
```

预期：零号消融 `2 encoder × 3 seed = 6`；主线 `2 backbone × 3 seed ×（1+1+3+3）= 48`；N1 代表档 `3 seed = 3`。每次 dry-run 同时生成 manifest CSV，记录 `tag/label/time_backbone/high_hz/encoder_type/seed/overrides`。
...
代码与测试全绿后，按规格“实验编排”执行。默认串行；如果单卡显存实测允许，可显式使用 `--max-parallel N --device cuda:0` 在同一 GPU 上并发多个训练进程。正式 CUDA 命令按项目规则提权执行。
...
1. **E0 收尾**：
   ...
   - 补跑 ssm：`python scripts/run_20260620_softz_candidates.py --only downsampled_ssm1d`。
2. **N1 频带尺度预统计**（仅 N1 对照档需要）：
   - `python scripts/precompute_stft_band_scale.py --config configs/tho_research_v2.yaml --set data.max_train_windows=null --set data.train_sample_seed=20260610 --high-hz 3 --output runs/stft_band_scale/band_scale_3hz.npy`
3. **E1 零号消融**（选编码器，6 run）：`python scripts/run_e1_stft_info_gain.py --phase zero --max-parallel <N>`，
   汇总后按主护栏 + 低 SNR 分层定 `conv1d` 或 `conv2d`（打平取 conv1d）。
4. **E1 主线**（48 run）：`python scripts/run_e1_stft_info_gain.py --phase main --encoder <胜者> --max-parallel <N>`，支持 `--skip` 续跑。
5. **N1 对照**（3 run）：`python scripts/run_e1_stft_info_gain.py --phase n1 --encoder <胜者> --band-scale-path runs/stft_band_scale/band_scale_3hz.npy --max-parallel <N>`。
6. **汇总**：`python scripts/summarize_tho_runs.py --runs-root runs/tho_research_v2_20260620_e1_stft_info_gain --output runs/tho_research_v2_20260620_e1_stft_info_gain_summary.csv`；同时保留 runner 生成的 manifest，用于把时间戳 run 目录回连到 `label/time_backbone/high_hz/encoder_type/seed`。
7. 按规划“记录模板”填 E1a/a'/b/c/d 指标 + 分层诊断，给出研究判定。
...
- E1 编排（零号消融 6 run 选编码器 + 主线 48 run + N1 对照 3 run，E1a 不经包装器、E1a' 容量桩、三频带、manifest 可追踪）→ 任务 10 ✓
- 测试策略（向后兼容护栏、契约、三模式、集成）→ 任务 1-8 ✓
- 验收（全量回归、零变化）→ 任务 11 ✓

**占位符扫描：** 无 TODO/待定；每个代码步骤均含完整代码。

**类型一致性：** `return_features` 返回 `(features, length)` 在任务 1/2 定义、任务 5 消费一致；`STFTEncoder` 构造参数（`out_channels`/`encoder_type`/`norm`/`band_scale_path`）在任务 3/6/7/10 一致；`TimeStftDual1D` 构造参数（`time_feat_channels`/`stft_kwargs.out_channels`）在任务 5/7 一致，且 multiscale 主干按 `base_channels * len(downsample_factors)` 推断；`build_run_specs` 返回的 `label`（E1a/E1a_prime/E1b/E1c）与 `build_n1_specs` 返回的 `E1n1` 在任务 10 测试与实现一致。
