...
- `filter_unusable=true`：训练和验证默认过滤 `usable=false` 窗口。
- `filter_unusable=false`：保留全部窗口，用于后续敏感性分析。

`audit.csv` 默认按 `split/input_set/residual_quality_class` 汇总：

- 窗口数。
- 可用窗口数。
- 可用窗口比例。
- 平均和最小 `valid_ratio`。
- 平均 `input_finite_ratio` 和 `target_finite_ratio`。
- `base_alignment_method` 主要取值。
- `apply_decision` 主要取值。

审计结果属于诊断产物，不作为首版硬门禁。若过滤后窗口数偏少，应在日志中记录警告、过滤前后窗口数和主要原因，但默认继续训练。
...
- 对 `tho_ref` 和 `r_tho_hat(t)` 同时计算频谱主峰读数和峰谷检测读数。
- 频谱主峰和峰谷检测是从“呼吸估计信号”中读出指标的工具，不代表原始 BCG 在所有窗口上都能被规则法正确解释。
- 对平凡基线输出也计算同类读数，但只作为弱基线诊断。
- 所有 RR/主频指标默认按 `residual_quality_class` 分层报告；低质量窗口规则读数失败是预期现象，应记录而不是视为流程错误。
...
- 数据覆盖统计，例如按 `split/input_set/residual_quality_class` 汇总窗口数和可用率。
...
该基线默认只运行在 `val` 子集，并与模型验证使用同一批窗口。该基线不用于证明模型优于传统方法，只用于暴露评价管线、标签读取或频带设置中的明显问题。基线异常属于诊断信息，应写入日志和 `baseline_metrics.csv`，默认不阻止训练。
...
1. 检查 `.venv` 中依赖是否可导入。
2. 数据审计 smoke test：读取索引并生成 `audit.csv`，记录可用窗口数量、质量分层和过滤结果。
3. Dataset smoke test：读取 `mixed_zscore` 的少量 train/val 窗口，确认形状、finite ratio 和 key 解析正确。
4. 平凡基线 smoke test：在同一验证子集生成 `baseline_metrics.csv`，记录 RR/主频和包络读数；异常仅作为诊断提示。
