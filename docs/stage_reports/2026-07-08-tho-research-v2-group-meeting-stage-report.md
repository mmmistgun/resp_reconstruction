...
本文中的 BCG 指同时包含心冲击、呼吸运动和体动等成分的体表机械信号；THO 指胸带记录的
胸廓呼吸波形，是当前重建任务的监督目标。
它只服务当前 THO research v2 soft-z 主线的阶段判断。下文所说的 soft-z，是指信号经过
分段稳健标准化后，再对极端幅值进行软压缩的表示；具体处理见“数据集与预处理”一节。
...
这里的 STFT 是由 BCG 输入在线计算的短时傅里叶变换，用于描述频率成分随时间的变化。
当前证据支持以宽频、时间分辨率更高的 STFT 辅助信息作为本阶段基准方案
（内部代号 `G3_C_wide_8p0`）；频带能量特征方案（内部代号 `G3_C_bandenergy`）保留为后续选择性修正的候选，
但暂不替代宽频 STFT。2026-07-09 `local_rr_*` 复评已覆盖当前比较方案，
结论仍支持这一判断。进一步的 THO 参考离线分析显示，四类网络都能在多数 BCG 二次谐波显著窗口中
抑制倍频并恢复目标基频；其中频带能量特征方案的呼吸率和周期计数误差最低，
宽频 STFT 方案的时延校正后形态相关性更高。这一结果支持保留频带能量特征作为选择性修正候选，
...
本文所说的当前主线，是指 THO research v2 soft-z 实验设置：
...
- 正式比较：固定数据划分、验证窗口、数据采样设置、独立训练次数和模型训练设置。
...
| 时序主干 | 同一 `patch_mixer1d` 时序主干和同一输出形式 |
| 训练设置 | 同一损失函数、优化器、训练轮数、批量大小、早停设置和独立训练次数 |
...
导出包的清单记录数据制作仓库代码版本为 `7ab93efa0d4c8394b0fdf7c57972f75d2a98eec8`；
导出时 `configs/default.yaml` 等文件仍有未提交修改。因此，本节优先依据导出包来源记录目录
`provenance/` 中保存的配置快照和实际导出信息，不直接套用当前数据制作仓库的工作区状态。
...
这个派生量的单位是 bpm，可理解为整窗平均意义上的周期计数误差；它不等同于稳健整窗呼吸率
或局部呼吸率。
...
当前实现中，`MA_long` 使用约 40 秒的平滑窗口，是 20 秒趋势参数的 2 倍。这里使用对数比例，
...
并根据频谱主峰采用更严格的寻峰间距，降低同一呼吸周期内双峰被误计为两个周期的概率。
计算过程是：
...
快慢变化趋势更同步。它仍不是逐呼吸事件匹配指标：若 `local_rr_valid_frac` 很低，或目标 THO 的局部
RR 曲线本身出现突变、谐波频率被误识别为主峰或低质量片段，仍需回到波形复核。旧版 20 秒窗口、
5 秒步长的寻峰口径和 v3 过零检查指标已退出当前评价，不再进入汇报排序。
...
可选辅助分支：
  同一 BCG 波形 -> 在线 STFT / 频带能量 -> 时频特征序列

融合：
  时频特征在时序混合模块之前与时序特征融合（实现位置为 pre_mixer）
  -> 后续时序混合
...
- 其余三个方案在同一时序主干上加入时频辅助信息，并在时序混合模块之前融合（实现位置为 `pre_mixer`）。
- 当前比较不改变输出形式，仍然回归 THO-like soft-z 波形。
...
线性降到 `0`。这里的 `warmup` 指训练早期对方向一致性的额外约束逐步减弱，不是学习率预热。
...
正式训练配置已保存在各次训练目录的 `config.yaml` 中。当前四类比较方案使用相同训练设置：
...
1. STFT 输入的时间分辨率是否应该从原来的 30 秒窗、5 秒步长调整。
...
时延校正后形态；局部 RR 也略有改善，但幅度明显小于旧版口径下的差异。
...
测试集复评结果上，又基于逐窗口指标做了离线分层分析。该分析不重新推理、
不重新评价模型权重、不覆盖历史训练结果；它按 `dataset_row_id` 对齐各方案的可用窗口，直接统计
每个分层中各模型的原始指标均值，不以方案间差值作为汇报结果。分层汇总输出在：

```text
runs/test_eval_g_series_20260709_local_rr_canonical/stratified_analysis_20260709/
```

### 参考模型高误差、低误差与低频谱相似度窗口

为了直接横向比较，这里用纯时序方案 `g0_time_only` 作为窗口分层的参考模型，
但它不是本阶段基准方案。参考模型高误差窗口（输出标签为 `baseline hard`）是指
`rr_peak_band_robust_abs_error > 1.0 bpm`；参考模型低误差窗口（`baseline easy`）是指同一指标
`<= 0.25 bpm`。同一分层内各模型对应同一批窗口。低频谱相似度窗口（`low spectrum`）是指
`g0_time_only` 输出与目标 THO 的 `spectrum_similarity` 不高于本次结果的中位数。三种分层
都使用了目标侧评价信息，因此只用于回顾性理解收益出现的位置，不能直接作为推理时的窗口选择条件。

| 分层 | 模型 | 窗口数 | 稳健 RR 误差 | 周期计数 bpm 误差 | 4 秒时延校正相关 | 局部 RR MAE |
|---|---|---:|---:|---:|---:|---:|
| 全部窗口 | `g0_time_only` | 2310 | 0.7732 | 0.7783 | 0.8653 | 0.7867 |
...
| 参考模型高误差 | `g0_time_only` | 308 | 4.3790 | 1.8908 | 0.7375 | 2.5755 |
...
| 参考模型低误差 | `g0_time_only` | 1396 | 0.0968 | 0.4158 | 0.9042 | 0.3528 |
...
| 低频谱相似度 | `g0_time_only` | 1155 | 1.3275 | 1.0742 | 0.8094 | 1.2631 |
...
这张表的核心读法是：宽频 STFT 方案 `G3_C_wide_8p0` 的整体稳健 RR 误差最低，从纯时序方案的
`0.7732` 降到 `0.7122`；收益主要来自参考模型高误差和低频谱相似度窗口。在参考模型高误差窗口中，
宽频 STFT 方案的稳健 RR 误差为 `3.8079`，低于上一版 STFT 方案的 `3.8786` 和频带能量特征方案的
`3.9029`；在低频谱相似度窗口中，宽频 STFT 方案的稳健 RR 误差同样最低。频带能量特征方案的优势
主要体现在周期计数和局部 RR：整体周期计数误差为 `0.6848 bpm`，局部 RR MAE 为 `0.7733 bpm`，
在参考模型高误差窗口中的周期计数误差也最低。但在参考模型低误差窗口中，所有时频方案的稳健 RR
误差都高于纯时序方案。这提示后续设计选择性修正时，应保护参考模型已经恢复准确的窗口；具体判据
仍需完全由 BCG 输入构造，不能依赖目标 THO。

### 周期计数失败窗口

周期计数分层用 `g0_time_only` 的 `breath_count_zero_cross_abs_error` 定义。
参考模型周期计数错误窗口（输出标签为 `baseline count-error > 0`）是指参考模型已经数错呼吸周期数；
参考模型周期计数正确窗口（`baseline count-error = 0`）是指参考模型的周期数完全正确。这个分层提示，
后续如果做选择性修正，应优先保护周期计数已经正确的窗口，并验证周期计数错误或结果不确定的窗口
是否更适合采用较强修正。

| 分层 | 模型 | 窗口数 | 稳健 RR 误差 | 周期计数 bpm 误差 | 局部 RR MAE |
|---|---|---:|---:|---:|---:|
| 参考模型周期计数错误 | `g0_time_only` | 1259 | 1.2386 | 1.4277 | 1.1846 |
...
| 参考模型周期计数正确 | `g0_time_only` | 1051 | 0.2157 | 0.0000 | 0.3104 |
...
在参考模型周期计数错误的窗口中，宽频 STFT 方案的稳健 RR 误差最低，为 `1.1453 bpm`；
频带能量特征方案的周期计数误差和局部 RR MAE 最低，分别为 `1.2008 bpm` 和 `1.1565 bpm`。
这提示频带能量特征可能有助于修正周期数量和局部 RR 长尾，但不一定带来最低的整窗 RR 误差。
在参考模型周期计数正确的窗口中，纯时序方案的周期计数误差原本就是 `0.0000 bpm`，所有时频方案
都会引入非零误差。因此，周期计数错误窗口可以作为候选修正对象，而周期计数正确窗口需要保护。
前者能否成为实际修正入口，仍需用不依赖目标信息的判据验证。

### RR 分层

RR 分层不是只考察较快呼吸，而是按目标 THO 的整窗呼吸率分成慢、中、偏快几档。脚本优先使用
`target_rr_peak_band_robust_bpm`，缺失时才回退到旧 RR 列。当前测试集中没有 `rr_ge24`
档样本，因此本轮只解释 `<10`、`10-14`、`14-18` 和 `18-24 bpm`。

| RR 档 | 模型 | 窗口数 | 稳健 RR 误差 | 周期计数 bpm 误差 | 相对包络相关 | 局部 RR MAE |
|---|---|---:|---:|---:|---:|---:|
...
RR 分层显示：宽频 STFT 方案在 `<10 bpm` 和 `14-18 bpm` 的稳健 RR 误差最低，说明它在慢呼吸和
中等呼吸率窗口中的表现更稳定；在 `10-14 bpm` 和 `18-24 bpm` 中，频带能量特征方案的稳健 RR
误差更低，局部 RR MAE 也最低。不过，频带能量特征方案在 `<10 bpm` 的稳健 RR 误差为 `4.0787 bpm`，
周期计数误差为 `1.4411 bpm`，均接近或劣于纯时序方案。因此，它更适合作为按条件使用的候选特征，
不适合统一替代宽频 STFT；具体选择条件仍需在下一阶段独立验证。

## 关键证据 5：BCG 二次谐波显著窗口大多可以恢复，但不是时频分支独有能力

前面的参考模型高误差 / 低误差分层从模型误差出发，不能直接说明输入 BCG 为什么难。为了更贴近“原始 BCG
呼吸节律不对，但其中双峰或倍频结构仍可能被网络恢复”的问题，本轮又建立了一个 THO 参考的离线
先验分层。该分析只回答“模型在这类已知困难窗口上表现如何”，不参与训练、不重新选择模型权重，
也不构造推理阶段的选择条件。
...
- 二倍频 / 基频邻域能量比不低于 `0.5393`，且二倍频占全分析带能量不低于 `0.1647`，
  记为谐波能量显著证据。
- 两类证据都成立的窗口称为“强二次谐波窗口”（`strong_harmonic`）；只有主峰倍频证据的窗口称为
  “主峰倍频窗口”（`peak_doubling`）；只有谐波能量证据的窗口称为“谐波能量显著窗口”
...
在定义更严格的 214 个强二次谐波窗口（`strong_harmonic`）中，差异更加清楚：
...
作为参照，1472 个未达到二次谐波显著标准的窗口（`harmonic_negative`）上的五项任务指标如下：
...
2. 高纠正率不等于呼吸任务已经完全解决。尤其在强二次谐波窗口中，纯时序方案和上一版 STFT 方案的周期计数
   误差仍超过 `3.4 bpm`。因此必须同时看 RR、周期计数、局部 RR 和形态指标，不能只报纠正状态。
3. 频带能量特征方案在阳性并集和强二次谐波窗口中的稳健 RR、周期计数与局部 RR 误差均最低；
   宽频 STFT 方案的时延校正后形态相关性最高。这与整体测试集上的结果一致：频带能量特征适合作为
   选择性修正候选，宽频 STFT 仍是综合表现更稳定的本阶段基准方案。
4. 在未达到二次谐波显著标准的参照窗口中，四类模型的差异明显收窄，频带能量特征方案也不再占优；
   这进一步说明，它的价值更可能集中在特定困难形态，而不适合在全部窗口中统一替代宽频 STFT。
...
统计边界也很明显：452 个阳性窗口中有 285 个来自受试者 671；214 个强二次谐波窗口中有
...
`local_rr_*` 口径，不重新训练，也不重新选择模型权重。复评目的不是重排所有做过的实验，
而是确认当前阶段结论在独立测试集上是否仍成立：

1. `G3_C_wide_8p0` 是否仍优于 `G0_time_only` 和 `G0_f0_native_stft_pre_mixer`。
2. 频带能量特征方案 `G3_C_bandenergy` 是否仍表现为“部分指标误差更低，但综合表现不如宽频方案稳定”。
...
4. 回顾性分层显示，宽频 STFT 在参考模型高误差、低频谱相似度和周期计数错误窗口中有较明显的
   收益；频带能量特征的正向结果也集中在困难窗口或部分 RR 区间，但会使参考模型低误差窗口、
   慢呼吸窗口和部分受试者的结果变差。
   这些结果揭示了选择性修正的潜在收益，尚不能直接证明可部署的选择条件。
5. 在 THO 参考定义的 BCG 二次谐波显著窗口中，四类网络都能在多数窗口抑制倍频并恢复基频，
   说明这是一种网络共有的可恢复能力；频带能量特征方案的呼吸率和周期计数误差最低，宽频 STFT
   方案的时延校正后形态相关性最高。该结果支持将频带能量特征作为选择性修正候选，但不改变宽频
   STFT 方案的本阶段基准地位。
6. 本次测试集复评只覆盖当前比较方案，不补不服务当前问题的分支。
7. 下一阶段关键问题从“继续增加输入信息”转向“什么时候使用辅助时频信息”。
...
- 当前参考模型高 / 低误差、周期计数错误和低频谱相似度分层均含有目标侧评价信息；它们只用于事后理解，不能直接作为推理时的窗口选择规则。
- BCG 二次谐波分层同样使用 THO 参考频率，只是离线先验分析，不是仅依赖 BCG 的质量检测器，也不能作为推理时的选择条件。
- 二次谐波阳性窗口集中在少数受试者，且主峰倍频层只有 5 个窗口；纠正率和模型差异不能外推为跨受试者稳定结论。
...
- 在更多受试者或受试者级重采样下验证二次谐波分层，重点确认频带能量特征在强二次谐波窗口中的收益是否稳定。
- 验证频带能量特征或高频信息是否能作为按条件使用的修正信息，而不是在全部窗口中无差别融合；
  若未来要做在线选择，再另行设计完全由 BCG 输入计算的无泄漏判据。
- 重新思考输出目标：从单一波形回归，逐步转向潜在呼吸状态、局部呼吸率、呼吸事件或不确定性辅助目标。
...
7. **整体任务流程与模型结构**：BCG 进入时序主干；可选的 STFT / 频带能量分支在时序混合模块之前
   融合（实现位置为 `pre_mixer`）；最终输出 THO-like soft-z 波形。
...
11. **独立测试集整体结果**：展示四方案主结果表，标明各指标的优劣方向，并说明宽频 STFT 与
    频带能量特征方案各自的优势。
12. **证据一——STFT 确实补充了纯时序信息**：比较纯时序、上一版 STFT 和宽频 STFT 方案，
    说明当前问题已经从“是否需要时频信息”转向“哪种表示更适合作为基准”。
13. **证据二——宽频 STFT 更适合作为当前基准**：宽频 STFT 的稳健 RR 误差更低，时延校正形态和
    综合表现更稳定；频带能量特征方案的周期计数和局部 RR 误差略低，但在慢呼吸和部分窗口上存在退化。明确两者分别是
    “本阶段基准方案”和“选择性修正候选特征”。
14. **长尾与严重错误**：补充稳健 RR 误差的 p95、误差超过 1 / 2 bpm 的窗口比例、周期计数误差 p95
    和局部 RR MAE p95；说明宽频 STFT 更稳定地降低稳健 RR 误差长尾，频带能量特征方案更明显地降低
    周期计数和局部 RR 误差长尾。

### 第四部分：收益发生在哪里

15. **分层分析的定位**：分层只用于回顾性理解收益来源；参考模型高 / 低误差、周期计数错误和
    低频谱相似度分层均含目标侧评价信息，不能直接作为推理时的选择条件。
16. **参考模型高 / 低误差与低频谱相似度窗口**：宽频 STFT 的收益主要来自参考模型高误差和
    低频谱相似度窗口；参考模型低误差窗口可能因加入时频信息而退化，说明选择性修正必须保护已正确窗口。
17. **周期计数错误与正确窗口**：参考模型已有周期计数错误的窗口中，频带能量特征呈现修正价值；
    周期计数正确的窗口中，时频方案可能引入少量非零计数误差。
18. **不同 RR 区间的表现差异**：宽频 STFT 在 `<10 bpm` 和 `14--18 bpm` 的表现更稳定；频带能量
    特征方案在 `10--14 bpm` 和 `18--24 bpm` 有局部优势，但不适合统一替代宽频 STFT。

### 第五部分：BCG 二次谐波困难窗口

19. **为什么单独研究二次谐波**：从输入信号机制解释倍频 / 双峰问题；说明这类分层比单纯按模型
    误差定义困难窗口更接近可观察的输入困难，但当前定义仍依赖 THO 参考。
20. **冻结规则、覆盖率与样本分布**：说明强二次谐波、主峰倍频和谐波能量显著三类窗口，
    阳性并集为 452 个窗口、占全部测试窗口 19.57%；同时展示阳性窗口和强二次谐波窗口集中在少数
    受试者的问题。
21. **四模型在阳性窗口上的结果**：四模型谐波纠正率均约为 95% 或以上；频带能量特征方案的稳健 RR、
    周期计数和局部 RR 误差最低，宽频 STFT 的时延校正后形态相关性最高。强调倍频恢复是网络共有能力，不是 STFT
    分支独有能力。
...
23. **阶段决策**：宽频 STFT 确定为本阶段基准；频带能量特征保留为选择性修正候选；不补评非重点
    历史分支，不继续无差别增加输入信息。
24. **证据边界与风险**：当前结论只适用于 research v2 soft-z；统计单位首先是重叠窗口而非独立
    受试者；二次谐波阳性集中在少数受试者；当前分层不能直接作为无泄漏选择条件；波形回归可能不是
    最终输出目标。
25. **下一阶段与三条带走结论**：建立仅依赖 BCG 的难度 / 不确定性判据；验证“宽频 STFT 基准 +
    按条件使用频带能量特征修正”；做受试者级复核或重采样；探索潜在呼吸状态、局部 RR、呼吸事件或
    不确定性辅助目标。页面结尾再次总结：STFT 值得保留、宽频 STFT 是当前基准、频带能量特征用于条件修正探索。
...
- 训练配置细节：损失权重、训练早期权重衰减安排（`warmup`）、早停、方向一致性筛选条件和 3 次独立训练。
- 指标公式：稳健带通 RR、周期计数、4 秒时延校正相关、相对包络和局部 RR。
- 分层分析细节：完整的参考模型高 / 低误差、周期计数错误和 RR 分层表，8 名受试者直接结果表，
  二次谐波子层完整结果、覆盖率和分析清单。
- 数据导出包来源记录：代码版本、配置快照和实际导出信息。
- 复现实验命令和输出路径。
...
这里的长尾指逐窗口误差分布的高端部分，主要看 p95 和超过固定阈值的窗口比例；严重错误
指稳健 RR 误差超过 `1 bpm` 或 `2 bpm` 的窗口。长尾统计显示，宽频 STFT 方案 `G3_C_wide_8p0`
的稳健带通 RR 严重错误比例最低，而频带能量特征方案 `G3_C_bandenergy` 的周期计数和局部 RR
误差长尾更低。

| 方案 | 稳健 RR 误差均值 | 稳健 RR 误差 p95 | 误差 >1 bpm 的窗口比例 | 误差 >2 bpm 的窗口比例 | 周期计数误差均值 | 周期计数误差 p95 | 局部 RR MAE p95 |
|---|---:|---:|---:|---:|---:|---:|---:|
...
表内每项均按“`g0_time_only` / `g3_c_wide_8p0`”顺序直接列出，不表示两方案之差。

| 受试者 | 窗口数 | 稳健 RR 误差 | 周期计数 bpm 误差 | 4 秒时延校正相关 | 局部 RR MAE |
|---:|---:|---:|---:|---:|---:|
...
宽频 STFT 在 8 名测试受试者上的稳健 RR 误差均低于纯时序方案，说明当前整体收益并非由单一受试者拉动；
但周期计数误差、4 秒时延校正相关和局部 RR MAE 的受试者内表现并不完全一致，仍需结合波形复核理解。
...
BCG 二次谐波分层的完整候选发现、验证图复核、阈值冻结、测试集应用、预测导出和纠正汇总命令见
...
其中 12 个模型权重文件的预测导出需要长时间占用 GPU，由用户手动执行；分析脚本只读取保存后的
